...
try:
    cnt = spark.table(f"`{CATALOG}`.silver.customers").count()
    if cnt > 0:
...
(quarantine_df.write
    ...
    .saveAsTable(f"`{CATALOG}`.silver.quarantine_customers"))
...
(resolution_audit.write
    ...
    .mode("overwrite")
    .option("overwriteSchema", "true")
    .saveAsTable(f"`{CATALOG}`.silver.customer_resolution_audit"))
...
if dq_entries:
    ...
    (dq_df.write
        ...
        .mode("overwrite")
        .option("overwriteSchema", "true")
        .saveAsTable(f"`{CATALOG}`.silver.dq_issues_customers"))
    print(f"logged {len(dq_entries)} quality issues")

# COMMAND ----------

display(spark.table(f"`{CATALOG}`.silver.dq_issues_customers"))
