...
try:
    cnt = spark.table("`databricks-hackathon-insurance`.silver.sales").count()
    if cnt > 0:
...
bronze_sales = spark.table("`databricks-hackathon-insurance`.bronze.sales")
...
(quarantine.write
    ...
    .saveAsTable("`databricks-hackathon-insurance`.silver.quarantine_sales"))
...
(silver_sales.write
    ...
    .mode("overwrite")
    .option("overwriteSchema", "true")
    .saveAsTable("`databricks-hackathon-insurance`.silver.sales"))
...
if dq_entries:
    ...
    (dq_df.write
        ...
        .mode("overwrite")
        .option("overwriteSchema", "true")
        .saveAsTable("`databricks-hackathon-insurance`.silver.dq_issues_sales"))
    display(dq_df)
