...
try:
    cnt = spark.table(f"`{CATALOG}`.silver.claims").count()
    if cnt > 0:
...
bronze_claims = spark.table(f"`{CATALOG}`.bronze.claims")
...
(quarantine.write.format("delta").mode("overwrite")
    ...
    .saveAsTable(f"`{CATALOG}`.silver.quarantine_claims"))
...
if dq_entries:
    ...
    (dq_df.write.format("delta").mode("overwrite")
        .option("overwriteSchema", "true")
        .saveAsTable(f"`{CATALOG}`.silver.dq_issues_claims"))
    display(dq_df)
else:
