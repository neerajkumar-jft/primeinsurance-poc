...
try:
    cnt = spark.table(f"`{CATALOG}`.silver.cars").count()
    if cnt > 0:
...
bronze_cars = spark.table(f"`{CATALOG}`.bronze.cars")
...
(quarantine.write
    ...
    .saveAsTable(f"`{CATALOG}`.silver.quarantine_cars"))
...
(silver_cars.write
    ...
    .mode("overwrite")
    .option("overwriteSchema", "true")
    .saveAsTable(f"`{CATALOG}`.silver.cars"))
...
if dq_entries:
    ...
    (dq_df.write
        ...
        .mode("overwrite")
        .option("overwriteSchema", "true")
        .saveAsTable(f"`{CATALOG}`.silver.dq_issues_cars"))
    display(dq_df)
else:
