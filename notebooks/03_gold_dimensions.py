...
silver_policy = spark.table(f"`{CATALOG}`.silver.policy")
...
(dim_policy.write
    ...
    .mode("overwrite")
    .option("overwriteSchema", "true")
    .saveAsTable(f"`{CATALOG}`.gold.dim_policy"))
...
silver_cars = spark.table(f"`{CATALOG}`.silver.cars")
...
(dim_car.write
    ...
    .mode("overwrite")
    .option("overwriteSchema", "true")
    .saveAsTable(f"`{CATALOG}`.gold.dim_car"))
