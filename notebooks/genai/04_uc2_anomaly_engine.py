...
# MAGIC %md
# MAGIC # UC2: Claims Risk and Anomaly Engine
# MAGIC
# MAGIC **Problem**: PrimeInsurance's claims investigators manually review 3,000 claims looking for
# MAGIC potential fraud. There is no consistent scoring, no prioritized queue, and no structured
# MAGIC explanation of why a claim is suspicious. Today this is done entirely by intuition.
# MAGIC
# MAGIC **Solution**: A statistical anomaly detection model using PySpark that scores every claim
# MAGIC against 5 weighted fraud-indicator rules, filters for anomalous claims, and generates
# MAGIC AI investigation briefs for each one.
# MAGIC
# MAGIC **Key principle**: The LLM is not a classifier -- it is an explainer.
# MAGIC Statistics detect. The LLM explains.
# MAGIC
# MAGIC **Architecture decisions**:
# MAGIC - 5 rules with weights totaling 100: z-score (25), IQR mismatch (25), no police (20), no witnesses (15), single vehicle total loss (15)
# MAGIC - Robust Z-score using MAD instead of standard deviation for skewed claim data
# MAGIC - IQR computed per severity band so thresholds adapt to the data
# MAGIC - Only HIGH and MEDIUM priority claims get sent to the LLM
# MAGIC - Pydantic validates all investigation briefs before writing to Gold
# MAGIC
# MAGIC **Input**: `{catalog}.silver.claims`
# MAGIC **Output**: `{catalog}.gold.claim_anomaly_explanations`
...
# Compute total_claim_amount from component columns
# Silver table has injury_amount, property_amount, vehicle_amount separately
# COALESCE handles nulls by treating them as 0
...
print(f"{len(flagged_claims)} claims flagged for investigation brief generation\n")
...
if results:
    for r in results:
        print("=" * 70)
        print(f"  [{r['priority']}] {r['claim_id']} | Score: {r['anomaly_score']}/100 | {r['priority']}")
