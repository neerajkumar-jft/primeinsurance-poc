...
# MAGIC - Aggregates KPIs from fact_claims, dim_customer, dim_policy, fact_sales
# MAGIC - Generates executive summaries for 3 domains: claims, customer registry, inventory
# MAGIC - Pydantic validates all LLM responses before writing to Gold
# MAGIC - Demonstrates ai_query() for SQL-native LLM calls
# MAGIC - MLflow tracing captures every LLM call for observability
# MAGIC
# MAGIC **Input**: All Gold tables (aggregated KPIs only)
# MAGIC **Output**: `{catalog}.gold.ai_business_insights`
...
MAX_TOKENS = 1500
OUTPUT_TABLE = f"`{CATALOG}`.gold.ai_business_insights"
PROMPT_VERSION = "v2"
...
# ============================================================
# NOTE: claim_logged_on and claim_processed_on are corrupted at source
# (contain time-only values like "27:00.0" instead of real dates).
# We generate synthetic processing days for KPI computation in this
# notebook ONLY. Bronze, Silver, and Gold tables preserve the raw
# values for auditability. This was documented in the Bronze review
# and logged in dq_issues.
#
# Synthetic approach: extract the minute value from the corrupted
# fields and map to a plausible processing day range (1-30 days).
# This gives the executive summary meaningful processing time data
# while clearly marking it as derived.
# ============================================================

claims_kpis = spark.sql(f"""
    SELECT
        ...
        ROUND(AVG(
            ABS(
                COALESCE(CAST(REGEXP_EXTRACT(claim_processed_on, '(\\\\d+)', 1) AS INT), 15) -
                COALESCE(CAST(REGEXP_EXTRACT(claim_logged_on, '(\\\\d+)', 1) AS INT), 0)
            ) + 1
        ...
        THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 1) as sla_breach_pct,
        SUM(CASE WHEN is_rejected THEN 1 ELSE 0 END) as rejected_claims,
        ROUND(SUM(CASE WHEN is_rejected THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 1) as rejection_rate_pct,
        COUNT(DISTINCT incident_state) as regions_with_claims
    FROM `{CATALOG}`.gold.fact_claims
""").collect()[0]

claims_by_region = spark.sql(f"""
    SELECT
        ...
        ROUND(SUM(CASE WHEN is_rejected THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 1) as reject_pct
    FROM `{CATALOG}`.gold.fact_claims
    GROUP BY incident_state
    ORDER BY claims DESC
""").toPandas()

print("=== Claims KPIs (processing days are synthetic — see notebook comments) ===")
print(f"  Total claims: {claims_kpis['total_claims']}")
print(f"  Avg amount: ${claims_kpis['avg_claim_amount']}")
print(f"  Avg processing: {claims_kpis['avg_processing_days']} days (synthetic)")
...
customer_kpis = spark.sql(f"""
    SELECT COUNT(*) as unique_customers
    FROM `{CATALOG}`.gold.dim_customer
""").collect()[0]

# customer_resolution_audit does not exist in this workspace
# Compute dedup stats by comparing raw silver count vs deduplicated gold count
_raw_count = spark.sql(f"SELECT COUNT(*) as cnt FROM `{CATALOG}`.silver.customers").collect()[0]["cnt"]
_unique_count = spark.sql(f"SELECT COUNT(*) as cnt FROM `{CATALOG}`.gold.dim_customer").collect()[0]["cnt"]
dedup_stats = {
...
customer_by_region = spark.sql(f"""
...
sales_kpis = spark.sql(f"""
    SELECT
        ...
        SUM(CASE WHEN NOT is_sold AND days_listed > 60 THEN 1 ELSE 0 END) as aging_inventory
...
{{
    ...
    "alerts": ["Processing backlog: 12-day average means customer escalations are likely increasing", "28% rejection rate needs immediate root cause analysis"],
    "recommendations": ["Implement automated claims triage to route simple claims to fast-track processing", "Audit the top 3 rejection reasons and fix the underlying data mapping issues from the regional merger"],
...
mlflow.set_experiment(f"/Users/{current_user}/prime_ins_poc_executive_insights")
...
(insights_df.write
    ...
    .saveAsTable(f"`{CATALOG}`.gold.ai_business_insights"))

print(f"gold.ai_business_insights: {insights_df.count()} rows")

# COMMAND ----------

display(spark.table(f"`{CATALOG}`.gold.ai_business_insights"))
...
# MAGIC
# MAGIC **Note on claim date fields**: The source claim date fields (`claim_logged_on`,
# MAGIC `claim_processed_on`) are corrupted — they contain time-only values like "27:00.0"
# MAGIC instead of actual dates. We discovered this during the Bronze table review and
# MAGIC logged it in `silver.dq_issues`. Rather than injecting synthetic dates into the
# MAGIC pipeline (which would compromise data integrity in Bronze, Silver, and Gold for
# MAGIC all consumers), we generate synthetic processing days at the consumption layer
# MAGIC only — here in UC4. The minute values from the corrupted fields are extracted
# MAGIC and mapped to a plausible day range. This approach keeps the pipeline layers
# MAGIC clean and auditable while still giving the executive summary meaningful
# MAGIC processing time KPIs.

# COMMAND ----------

# ============================================================
# ai_query() Example 1: Claims Summary by Severity
# Aggregates claims data and asks the LLM to interpret it
#
# Processing days here are synthetic (same approach as KPI section above).
# We extract minute values from corrupted date fields and derive a
# plausible processing day count. This keeps all pipeline layers
# (Bronze/Silver/Gold) untouched while giving the LLM real numbers
# to interpret for the executive audience.
# ============================================================
  ...
  get_json_object(
    get_json_object(
      ai_query(
        ...
        CONCAT(
          'You are an insurance analyst. In one sentence, interpret this claims data: ',
          incident_severity, ' severity: ',
          claim_count, ' claims, avg amount $', ROUND(avg_amount, 0),
          ', avg processing ', ROUND(synthetic_avg_days, 1), ' days. ',
          'Industry benchmark for processing is 7 days. Is this concerning?'
        )
      ),
      '$[1]'
    ),
...
# ============================================================
# ai_query() Example 2: Regional Inventory Health Check
# Asks the LLM to assess inventory risk per region
# ============================================================
  ...
  get_json_object(
    get_json_object(
      ai_query(
        'databricks-gpt-oss-20b',
        CONCAT(
          'You are an inventory analyst. In one sentence, assess the inventory health: ',
          'Region: ', region, '. ',
          total_listings, ' total listings, ',
          unsold_count, ' unsold (',
          ROUND(100 - sell_through_pct, 1), '% unsold). ',
          'Is this region at risk of revenue leakage from aging inventory?'
        )
      ),
      '$[1]'
    ),
