...
try:
    cnt = spark.table(f"`{CATALOG}`.silver.policy").count()
    if cnt > 0:
...
bronze_policy = spark.table(f"`{CATALOG}`.bronze.policy")
...
(quarantine.write
    ...
    .saveAsTable(f"`{CATALOG}`.silver.quarantine_policy"))
...
(silver_policy.write
    ...
    .mode("overwrite")
    .option("overwriteSchema", "true")
    .saveAsTable(f"`{CATALOG}`.silver.policy"))
...
if dq_entries:
    ...
    (dq_df.write
        ...
        .mode("overwrite")
        .option("overwriteSchema", "true")
        .saveAsTable(f"`{CATALOG}`.silver.dq_issues_policy"))
    display(dq_df)
else:
