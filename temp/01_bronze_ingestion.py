...
for t in required_tables:
    try:
        cnt = spark.table(f"`databricks-hackathon-insurance`.bronze.{t}").count()
        if cnt == 0:
            all_exist = False
            break
    except Exception:
...
(bronze_customers.write
    ...
    .saveAsTable("`databricks-hackathon-insurance`.bronze.customers"))
...
print("merged schema:")
spark.table("`databricks-hackathon-insurance`.bronze.customers").printSchema()
...
(bronze_claims.write
    ...
    .mode("overwrite")
    .option("overwriteSchema", "true")
    .saveAsTable("`databricks-hackathon-insurance`.bronze.claims"))

print(f"\nbronze.claims: {bronze_claims.count()} rows")

# COMMAND ----------

display(spark.table("`databricks-hackathon-insurance`.bronze.claims").limit(5))
...
(bronze_policy.write
    ...
    .mode("overwrite")
    .option("overwriteSchema", "true")
    .saveAsTable("`databricks-hackathon-insurance`.bronze.policy"))
...
(bronze_sales.write
    ...
    .mode("overwrite")
    .option("overwriteSchema", "true")
    .saveAsTable("`databricks-hackathon-insurance`.bronze.sales"))
...
(bronze_cars.write
    ...
    .mode("overwrite")
    .option("overwriteSchema", "true")
    .saveAsTable("`databricks-hackathon-insurance`.bronze.cars"))
...
# MAGIC %sql
# MAGIC SHOW TABLES IN `databricks-hackathon-insurance`.bronze;
...
for t in tables:
    tbl = spark.table(f"`databricks-hackathon-insurance`.bronze.{t}")
